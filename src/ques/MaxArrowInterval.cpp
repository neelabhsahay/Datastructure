/*
 * There are some spherical balloons taped onto a flat wall that
 * represents the XY-plane. The balloons are represented as a 2D
 * integer array points where points[i] = [xstart, xend] denotes
 * a balloon whose horizontal diameter stretches between xstart 
 * and xend. You do not know the exact y-coordinates of the balloons.
 *
 * Arrows can be shot up directly vertically
 *  (in the positive y-direction) from different points along the x-axis.
 *  A balloon with xstart and xend is burst by an arrow shot at x
 *  if xstart <= x <= xend. There is no limit to the number of arrows
 * that can be shot. A shot arrow keeps traveling up infinitely,
 * bursting any balloons in its path.
 *
 * Given the array points, return the minimum number of arrows
 * that must be shot to burst all balloons.
 *
 */

#include<iostream>
#include<vector>

using namespace std;

bool smaller(vector<int> &a, vector<int> &b ) {
    if(a[1] < b[1])
        return true;
    else if(a[0] == b[0] && a[0] < b[0] )
        return true;
    else
        return false;
}

class Solution {
public:
    int findMinArrowShots(vector<vector<int>>& points) {
        sort(points.begin(), points.end(), smaller);
        
        int pos = points[0][1];
        int arrows = 1;
        for( int i = 0 ; i < points.size(); i++) {
            if( points[i][0] <= pos)
                continue;
            arrows++;
            pos = points[i][1];
        }
        return arrows;
    }
};

int
main() {

  vector<vector<int>> points{{9,12},{1,10},{4,11},{8,12},{3,9},{6,9},{6,7}};
  Solution s;
  int coun = s.findMinArrowShots(points);
  cout << " Total Arrows: " << coun << endl;
  return 0;
}
