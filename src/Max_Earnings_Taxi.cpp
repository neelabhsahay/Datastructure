...
* a ride from point starti to point endi who is willing to give a tipi dollar tip.
*
...
public:
    long long maxTaxiEarnings(int n, vector<vector<int>>& rides) {
        ...
        for(int i =0; i< rides.size();i++ ) {
            rMap.insert(pair(rides[i][1],i) );
        }
        long long earn;
        vector<long long> ern(n+1,0);
        for(int i = 1;i <=n;i++) {
            multimap<int,int>::iterator it = rMap.find(i);
            if(it!= rMap.end()) {
                while( it!= rMap.end()) {
                    idx = it->second;
                    earn = rides[idx][2] + (rides[idx][1] - rides[idx][0]) + ern[rides[idx][0]];
                    if(ern[i] < earn)
                        ern[i] = earn;
                    rMap.erase (it);
                    it = rMap.find(i);
